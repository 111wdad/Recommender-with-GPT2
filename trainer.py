...
class Trainer:
    ...
    def _evaluate(self, eval_loader, k=[10, 20, 50]):
        ...
        metrics.update({f'Recall@{k_}': [] for k_ in k})
        metrics.update({f'Precision@{k_}': [] for k_ in k})
        ...
        for users in eval_loader:
            ...
            for idx, items in enumerate(pos_items):
                ...
                for k_ in k:
                    ...
                    num_pos = len(items)
                    hits = np.sum(hit_k)
                    recall_k = hits / num_pos if num_pos > 0 else 0
                    precision_k = hits / k_

                    metrics[f'NDCG@{k_}'].append(ndcg_k)
                    metrics[f'HR@{k_}'].append(hr_k)
                    metrics[f'Recall@{k_}'].append(recall_k)
                    metrics[f'Precision@{k_}'].append(precision_k)
        ...
        for epoch in tqdm(range(self.epochs), desc='Training'):
            ...
            if (epoch + 1) % self.eval_step == 0:
                ...
                self.logger.info(
                    ...
                    f'valid_recall@10: {valid_result["Recall@10"]:.4f} - '
                    f'valid_precision@10: {valid_result["Precision@10"]:.4f} - '
